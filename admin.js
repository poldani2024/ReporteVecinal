...
// ---------------------------------------------------------------------
// Inicialización (por si firebase.js no expone auth/db)
// ---------------------------------------------------------------------
try {
  // Si firebase.js ya los definió, esto no hace nada.
...
// Configuración: qué ve un vecino
const SHOW_ONLY_OWN_FOR_VECINO = true; // true => sólo sus propios reportes
...
function csvEscape(val) {
  if (val == null) return '';
  const s = String(val);
  // Si contiene comillas, coma o salto de línea, envolver y escapar
  if (/[",\r\n]/.test(s)) {
    return '"' + s.replace(/"/g, '""') + '"';
  }
  return s;
...
// ---------------------------------------------------------------------
// Render de fila (sin HTML embebido, seguro con textContent)
// ---------------------------------------------------------------------
function renderRow(doc) {
  ...
  // Si el usuario no es admin y no es dueño del reporte, deshabilitar delete
  ...
  btnDel.addEventListener('click', async () => {
    ...
    try {
      ...
      // onSnapshot se encargará de remover la fila.
    } catch (err) {
      console.error(err);
